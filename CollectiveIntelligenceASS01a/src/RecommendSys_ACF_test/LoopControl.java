...
class LoopControl {
	 ...
	 **********************************************************************/
	FinalResults LOOLoopTest(ResysHashMap resysHashMap, String NeighbourSelectMethod, float[][] DistanceMatrix){
		...
		FinalResults finalResults = finalResultsInit(300, 300, userMap, itemListMap);
        ...
        for (Iterator<Integer> user_iterator = user_set.iterator(); user_iterator.hasNext();) {
        	...
        	//display.change_log_file("Task3PredictResults_"+ NeighbourSelectMethod + ".csv");
        	for (Integer n = finalResults.n_start; n <= finalResults.n_stop;){
        		// Predict from n closest neighbours.
        		UserNPredictResult userPredictResult = new Prediction().predictFromNNeighbour
        							(User_id, n, neighbourIDSortedList, userMap, DistanceMatrix, NeighbourSelectMethod, predictResultLogFileHead);
        ...
        evaluation.OverallEvaluation(finalResults, NeighbourSelectMethod, "Task3_OverallEvaluation_");
	 ...
	 **********************************************************************/
	FinalResults LOOLoopTest_Resnick(ResysHashMap resysHashMap, String NeighbourSelectMethod, float[][] DistanceMatrix){
		long predict_startTime_ms = System.currentTimeMillis();
		System.out.println("LOOLoopTest: \"" + NeighbourSelectMethod + "\" Start!");
		...
		FinalResults finalResults = finalResultsInit(10, 10, userMap, itemListMap);
		...
		//System.out.println(usesAverageRatingMap.toString());
        ...
        for (Iterator<Integer> user_iterator = user_set.iterator(); user_iterator.hasNext();) {
        	...
        	// Check first n neighbours from start_n to stop_n.	(Loop-level:2, O(x)=stop_n-start_n)
        	display.change_log_file("Task4_PredictResults_"+ NeighbourSelectMethod + ".csv");
        	for (Integer n = finalResults.n_start; n <= finalResults.n_stop;){
        		// Predict from n closest neighbours.
        		UserNPredictResult userPredictResult = new Prediction().ResnickpredictFromNNeighbour
        							(User_id, n, neighbourIDSortedList, userMap, DistanceMatrix, usesAverageRatingMap, NeighbourSelectMethod, predictResultLogFileHead);
        		if(userPredictResult == null) {
        			continue;
        		}
        		if(UserNPredictResultMap.get(n) == null) {
        ...
        finalResults.TotalRunTime = predict_totalRunTome_ms;
        Evaluation evaluation = new Evaluation();
        evaluation.OverallEvaluation(finalResults, NeighbourSelectMethod, "Task4_OverallEvaluation_");
        System.out.println(String.format("\tLOOLoopTest \"%s\" Over, Total predict Time:%.4f ms, user_num:%d, n_num:%d", 
        									NeighbourSelectMethod, predict_totalRunTome_ms, 
        									finalResults.UsersPredictResultsMap.size(), finalResults.n_num));
        return finalResults;
