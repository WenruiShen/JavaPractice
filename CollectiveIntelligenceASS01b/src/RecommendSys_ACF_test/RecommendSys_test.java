...
class RecommendSys{
	...
	public RecommendSys(String file_name) throws IOException{
		...
		DiatanceSimCollaborativeFilterTest(resysHashMap, DistanceMatrix, ratings_Count);
		
		// Task_4: 
		System.out.println("<Task_4> start:");
		ResnickPrediction(resysHashMap, DistanceMatrix, ratings_Count);
	 ...
	 *******************************************************************/
	private void DiatanceSimCollaborativeFilterTest(ResysHashMap resysHashMap, float[][][] DistanceMatrix, Ratings_Count ratings_Count){
		LoopControl loopControl = new LoopControl();
		loopControl.LOOLoopTest(resysHashMap, "Distance", DistanceMatrix, ratings_Count);
		loopControl.LOOLoopTest(resysHashMap, "Size", DistanceMatrix, ratings_Count);
	 ...
	 **************************************/
	private void ResnickPrediction(ResysHashMap resysHashMap, float[][][] DistanceMatrix, Ratings_Count ratings_Count){
		LoopControl loopControl = new LoopControl();
		loopControl.LOOLoopTest_Resnick(resysHashMap, "Distance", DistanceMatrix, ratings_Count);
		loopControl.LOOLoopTest_Resnick(resysHashMap, "Size", DistanceMatrix, ratings_Count);
	}
